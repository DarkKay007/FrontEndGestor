...
const useAssignmentStore = create((set) => ({
  ...
  fetchAssignments: async () => {
    ...
    try {
      ...
      const response = await axios.get("https://backend-2ktb.onrender.com/api/asignaciones", {
        headers: {
  ...
  fetchUserNames: async (data) => {
    try {
      ...
      const token = Cookies.get('token');
      const response = await axios.get("https://backend-2ktb.onrender.com/api/usuario", {
